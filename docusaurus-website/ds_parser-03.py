...
class DSCourseParser:
    ...
    def _parse_course_section(self, section: str) -> Dict:
        ...
        for i, original_line in enumerate(lines):
            ...
            if 'اهداف درس:' in original_line:
                course_data['goals'] = self._extract_section_content(lines, i, 'اهداف درس:', next_markers=[
            ...
            if 'توانایی و شایستگی‌هایی که درس پرورش می‌دهد:' in original_line:
                course_data['competencies'] = self._extract_section_content(lines, i, 'توانایی و شایستگی‌هایی که درس پرورش می‌دهد:', next_markers=[
                    'سرفصل درس:',
                    'راهبردهای تدریس',
                    'روش یاددهی-یادگیری',
                    'روش یاددهی',
            ...
            if 'راهبردهای تدریس' in original_line:
                course_data['teaching_strategies'] = self._extract_section_content(lines, i, 'راهبردهای تدریس و یادگیری متناسب با محتوا و هدف:', next_markers=[
                    'روش یاددهی-یادگیری',
                    'روش یاددهی',
                    'روش ارزیابی',
                    'تجهیزات',
            ...
            if 'روش یاددهی-یادگیری:' in original_line:
                course_data['teaching_methods'] = self._extract_section_content(lines, i, 'روش یاددهی-یادگیری:', next_markers=[
                    'روش ارزیابی',
                    'تجهیزات',
                    'منابع:',
                    '| # | # | # | # | # |',
                ...
                course_data['teaching_methods'] = self._extract_section_content(lines, i, 'روش یاددهی:', next_markers=[
                    'روش ارزیابی',
                    'تجهیزات',
                    'منابع:',
                    '| # | # | # | # | # |',
            ...
            if 'روش ارزیابی:' in original_line:
                course_data['assessment_methods'] = self._extract_section_content(lines, i, 'روش ارزیابی:', next_markers=[
                    'تجهیزات',
                    'منابع:',
                    '| # | # | # | # | # |',
                    'عنوان درس به فارسی:'
            ...
            if 'تجهیزات' in original_line and 'تجهیزات و امکانات' not in original_line:
                course_data['equipment'] = self._extract_section_content(lines, i, 'تجهیزات', next_markers=[
                    'منابع:',
                    '| # | # | # | # | # |',
                    'عنوان درس به فارسی:'
                ])
    ...
    def _extract_section_content(self, lines: List[str], start_index: int, marker: str, next_markers: List[str]) -> str:
        ...
        for i in range(start_index, len(lines)):
            ...
            if marker in current_line and not marker_found:
                ...
                if len(parts) > 1 and parts[1].strip():
                    # حذف متن اضافی از انتهای خط (مثل مارکر بعدی)
                    clean_text = parts[1].strip()
                    for next_marker in next_markers:
                        if next_marker in clean_text:
                            clean_text = clean_text.split(next_marker)[0].strip()
                            break
                    if clean_text:
            ...
            if marker_found:
                ...
                if current_line.strip().startswith('|'):
                    # اما اگر جدول کوچک ارزیابی است، ادامه می‌دهیم
                    if '| # | # | # | # |' in current_line and len(current_line.split('|')) > 5:
                        # این جدول ارزیابی است، متوقف شو
                        break
                    continue
    ...
    def _extract_syllabus(self, lines: List[str], start_index: int) -> str:
        """استخراج سرفصل درس - به صورت متن ساده"""
        syllabus_lines = []
        in_syllabus = False
        markers_to_stop = [
            'راهبردهای تدریس',
        ...
        # ترکیب همه خطوط سرفصل
        combined_syllabus = ' '.join(syllabus_lines).strip()
        
        # حذف فاصله‌های اضافی اما حفظ ساختار متن
        combined_syllabus = re.sub(r'\s+', ' ', combined_syllabus)
        
        return combined_syllabus
    ...
    def _create_dataframe(self, courses_data: List[Dict]):
        ...
        columns = [
            ...
            'goals', 'competencies', 'syllabus', 'teaching_strategies',
            'teaching_methods', 'assessment_methods', 'equipment', 'references'
        ]
...
class DocusaurusMarkdownGenerator:
    ...
    def generate_files(self, df: pd.DataFrame):
        ...
        for idx, row in df.iterrows():
            ...
            df.at[idx, 'position'] = position_counter[course_type]
    ...
    def _create_file_content(self, row, folder: str) -> str:
        ...
        syllabus = str(row['syllabus']).strip() if str(row['syllabus']).strip() != 'nan' else ''
        teaching_strategies = str(row['teaching_strategies']).strip() if 'teaching_strategies' in row and str(row['teaching_strategies']).strip() != 'nan' else ''
        teaching_methods = str(row['teaching_methods']).strip() if 'teaching_methods' in row and str(row['teaching_methods']).strip() != 'nan' else ''
        assessment_methods = str(row['assessment_methods']).strip() if 'assessment_methods' in row and str(row['assessment_methods']).strip() != 'nan' else ''
        equipment = str(row['equipment']).strip() if 'equipment' in row and str(row['equipment']).strip() != 'nan' else ''
        ...
        if teaching_strategies:
            lines.append('### راهبردهای تدریس و یادگیری')
            lines.append('')
            lines.append(teaching_strategies)
            lines.append('')
        
        # روش یاددهی-یادگیری
        if teaching_methods:
            lines.append('### روش یاددهی-یادگیری')
            lines.append('')
            lines.append(teaching_methods)
            lines.append('')
        
        # روش ارزیابی
        if assessment_methods:
            lines.append('### روش ارزیابی')
            lines.append('')
            lines.append(assessment_methods)
            lines.append('')
        
        # تجهیزات
        if equipment:
            lines.append('### تجهیزات و امکانات موردنیاز')
            lines.append('')
